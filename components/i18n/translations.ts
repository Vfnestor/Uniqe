export type Language = "en" | "fa";

export const translations = {
  en: {
    language: "English",
    languageShort: "EN",

    theme: {
      dark: "Dark",
      light: "Light",
    },

    user: {
      account: "Account",
      profile: "Profile",
      activity: "Activity",
      notifications: "Notifications",
      favorites: "Favorites",
      dashboard: "My U",
      logout: "Log out",
      login: "Log in",
      register: "Create account",
      guest: "Guest",
    },

    navigation: {
      home: "Home",
      uapps: "UApps",
      uweb: "UWeb",
      ushop: "UShop",
      uschool: "USchool",
      ucore: "UCore",
      lab: "LAB",
      myU: "My U",
      search: "Search",
    },

    home: {
      ecosystem: "Digital Ecosystem",
      build: "Build.",
      connect: "Connect.",
      explore: "Explore.",
      description:
        "Uniqe is a digital ecosystem built to bring applications, web, commerce, education, technology and innovation together.",
      exploreUApps: "Explore UApps",
      enterLab: "Enter LAB",
      oneEcosystem: "One ecosystem",
      multiplePossibilities:
        "Multiple possibilities",
      scroll: "Scroll to explore",
    },

    common: {
      comingSoon: "Coming Soon",
      inDevelopment: "In Development",
      available: "Available",
      experimental: "Experimental",
      active: "Active",
      concept: "Concept",
      explore: "Explore",
      learnMore: "Learn more",
      viewAll: "View all",
      goHome: "Go home",
      tryAgain: "Try again",
      restart: "Restart",
      all: "All",
      close: "Close",
      cancel: "Cancel",
      save: "Save",
      search: "Search",
      clear: "Clear",
      noResults: "No results found",
    },

    text: {},
  },

  fa: {
    language: "فارسی",
    languageShort: "FA",

    theme: {
      dark: "تیره",
      light: "روشن",
    },

    user: {
      account: "حساب کاربری",
      profile: "پروفایل",
      activity: "فعالیت‌ها",
      notifications: "اعلان‌ها",
      favorites: "علاقه‌مندی‌ها",
      dashboard: "My U",
      logout: "خروج از حساب",
      login: "ورود",
      register: "ساخت حساب",
      guest: "مهمان",
    },

    navigation: {
      home: "خانه",
      uapps: "UApps",
      uweb: "UWeb",
      ushop: "UShop",
      uschool: "USchool",
      ucore: "UCore",
      lab: "LAB",
      myU: "My U",
      search: "جستجو",
    },

    home: {
      ecosystem: "اکوسیستم دیجیتال",
      build: "بساز.",
      connect: "متصل کن.",
      explore: "کشف کن.",
      description:
        "Uniqe یک اکوسیستم دیجیتال است که اپلیکیشن‌ها، وب، تجارت، آموزش، فناوری و نوآوری را در یک مجموعه به هم متصل می‌کند.",
      exploreUApps: "مشاهده UApps",
      enterLab: "ورود به LAB",
      oneEcosystem: "یک اکوسیستم",
      multiplePossibilities:
        "امکانات متعدد",
      scroll: "برای کشف بیشتر اسکرول کنید",
    },

    common: {
      comingSoon: "به‌زودی",
      inDevelopment: "در حال توسعه",
      available: "در دسترس",
      experimental: "آزمایشی",
      active: "فعال",
      concept: "ایده",
      explore: "مشاهده",
      learnMore: "بیشتر بدانید",
      viewAll: "مشاهده همه",
      goHome: "بازگشت به خانه",
      tryAgain: "تلاش دوباره",
      restart: "راه‌اندازی مجدد",
      all: "همه",
      close: "بستن",
      cancel: "لغو",
      save: "ذخیره",
      search: "جستجو",
      clear: "پاک کردن",
      noResults: "نتیجه‌ای پیدا نشد",
    },

    /*
     * Central translation dictionary.
     *
     * Existing pages currently contain many English
     * strings directly inside their components.
     *
     * Keeping them here lets the whole existing UI
     * switch language without rewriting every page.
     */
    text: {
      "Digital Ecosystem":
        "اکوسیستم دیجیتال",

      "Build.":
        "بساز.",

      "Connect.":
        "متصل کن.",

      "Explore.":
        "کشف کن.",

      "Explore UApps":
        "مشاهده UApps",

      "Enter LAB":
        "ورود به LAB",

      "One ecosystem":
        "یک اکوسیستم",

      "Multiple possibilities":
        "امکانات متعدد",

      "Scroll to explore":
        "برای کشف بیشتر اسکرول کنید",

      "Uniqe Vision":
        "چشم‌انداز Uniqe",

      "A digital ecosystem":
        "یک اکوسیستم دیجیتال",

      "built to evolve.":
        "ساخته‌شده برای تکامل.",

      "Connect":
        "اتصال",

      "Build":
        "ساختن",

      "Evolve":
        "تکامل",

      "Bring different digital experiences together inside one connected ecosystem.":
        "تجربه‌های دیجیتال مختلف را در یک اکوسیستم یکپارچه در کنار هم قرار دهید.",

      "Turn ideas into useful products, applications, services and experiences.":
        "ایده‌ها را به محصولات، اپلیکیشن‌ها، خدمات و تجربه‌های کاربردی تبدیل کنید.",

      "Keep improving the system as technology, people and possibilities change.":
        "با تغییر فناوری، انسان‌ها و امکانات، سیستم را به‌طور مداوم بهبود دهید.",

      "Uniqe is not a single product. It is a growing digital environment where ideas, technology and experiences can exist together.":
        "Uniqe یک محصول واحد نیست؛ یک محیط دیجیتال در حال رشد است که در آن ایده‌ها، فناوری و تجربه‌ها می‌توانند در کنار یکدیگر وجود داشته باشند.",

      "One ecosystem. Continuous evolution.":
        "یک اکوسیستم. تکامل مداوم.",

      "Applications":
        "اپلیکیشن‌ها",

      "Digital tools and applications designed for everyday use, productivity and everyday digital experiences.":
        "ابزارها و اپلیکیشن‌های دیجیتال برای استفاده روزمره، افزایش بهره‌وری و تجربه‌های دیجیتال کاربردی.",

      "UApps Core":
        "هسته UApps",

      "The central foundation for discovering and accessing applications across the UApps ecosystem.":
        "زیرساخت مرکزی برای کشف و دسترسی به اپلیکیشن‌های اکوسیستم UApps.",

      "Smart Tools":
        "ابزارهای هوشمند",

      "A collection of practical digital tools designed to make everyday tasks simpler.":
        "مجموعه‌ای از ابزارهای دیجیتال کاربردی برای ساده‌تر کردن کارهای روزمره.",

      "Automation":
        "اتوماسیون",

      "Tools and workflows designed to automate repetitive digital tasks and processes.":
        "ابزارها و جریان‌های کاری برای خودکارسازی وظایف و فرایندهای تکراری دیجیتال.",

      "UApps LAB":
        "آزمایشگاه UApps",

      "Experimental applications and prototypes exploring new ideas inside the UApps ecosystem.":
        "اپلیکیشن‌ها و نمونه‌های آزمایشی برای بررسی ایده‌های جدید در اکوسیستم UApps.",

      "Productivity":
        "بهره‌وری",

      "Utility":
        "ابزار کاربردی",

      "Experimental":
        "آزمایشی",


      "Websites":
        "وب‌سایت‌ها",

      "The central web layer connecting websites, platforms and digital experiences across the Uniqe ecosystem.":
        "لایه مرکزی وب که وب‌سایت‌ها، پلتفرم‌ها و تجربه‌های دیجیتال اکوسیستم Uniqe را به هم متصل می‌کند.",

      "Uniqe Web":
        "وب Uniqe",

      "Web Platforms":
        "پلتفرم‌های وب",

      "Connected web platforms designed to provide useful digital experiences across different services.":
        "پلتفرم‌های وب متصل برای ارائه تجربه‌های دیجیتال کاربردی در سرویس‌های مختلف.",

      "Digital Services":
        "خدمات دیجیتال",

      "Web-based services built to make digital tasks, interactions and experiences more accessible.":
        "خدمات مبتنی بر وب برای ساده‌تر و در دسترس‌تر کردن فعالیت‌ها و تجربه‌های دیجیتال.",

      "Web Experiments":
        "آزمایش‌های وب",

      "Experimental websites and prototypes exploring new interfaces, ideas and possibilities.":
        "وب‌سایت‌ها و نمونه‌های آزمایشی برای بررسی رابط‌ها، ایده‌ها و امکانات جدید.",

      "Platform":
        "پلتفرم",

      "Services":
        "خدمات",

      "Available":
        "در دسترس",

      "In Development":
        "در حال توسعه",

      "Coming Soon":
        "به‌زودی",

      "Digital":
        "دیجیتال",

      "Products":
        "محصولات",

      "Physical":
        "فیزیکی",

      "Uniqe Digital":
        "Uniqe Digital",

      "Digital products and resources designed to become useful parts of the wider Uniqe ecosystem.":
        "محصولات و منابع دیجیتال که برای تبدیل شدن به بخش‌های کاربردی اکوسیستم گسترده Uniqe طراحی شده‌اند.",

      "Uniqe Services":
        "Uniqe Services",

      "A foundation for discovering services and solutions connected to the Uniqe ecosystem.":
        "زیرساختی برای کشف خدمات و راهکارهای متصل به اکوسیستم Uniqe.",

      "Uniqe Products":
        "Uniqe Products",

      "Physical products and curated items that can become part of the future Uniqe marketplace.":
        "محصولات فیزیکی و اقلام منتخب که می‌توانند بخشی از بازار آینده Uniqe باشند.",

      "UShop LAB":
        "آزمایشگاه UShop",

      "Experimental commerce concepts exploring new ways to connect products, services and digital experiences.":
        "مفاهیم آزمایشی تجارت برای بررسی روش‌های جدید اتصال محصولات، خدمات و تجربه‌های دیجیتال.",

      "Technology":
        "فناوری",

      "Business":
        "کسب‌وکار",

      "Creative":
        "خلاقیت",

      "Foundation":
        "زیرساخت",

      "Learning paths focused on software, digital technology and the skills needed to build modern products.":
        "مسیرهای آموزشی متمرکز بر نرم‌افزار، فناوری دیجیتال و مهارت‌های لازم برای ساخت محصولات مدرن.",

      "Practical learning experiences exploring business, entrepreneurship, strategy and digital growth.":
        "تجربه‌های آموزشی کاربردی در زمینه کسب‌وکار، کارآفرینی، استراتژی و رشد دیجیتال.",

      "Courses and learning experiences for design, creativity, digital media and creative problem solving.":
        "دوره‌ها و تجربه‌های آموزشی در زمینه طراحی، خلاقیت، رسانه‌های دیجیتال و حل مسئله خلاقانه.",

      "USchool Foundation":
        "زیرساخت USchool",

      "The foundation layer for organizing learning experiences, educational resources and future USchool programs.":
        "لایه زیرساختی برای سازمان‌دهی تجربه‌های آموزشی، منابع یادگیری و برنامه‌های آینده USchool.",

      "Core Platform":
        "پلتفرم مرکزی",

      "The central foundation that connects and coordinates the different layers of the Uniqe ecosystem.":
        "زیرساخت مرکزی که لایه‌های مختلف اکوسیستم Uniqe را به هم متصل و هماهنگ می‌کند.",

      "Data Layer":
        "لایه داده",

      "A future foundation for organizing, managing and connecting data across Uniqe products and services.":
        "زیرساخت آینده برای سازمان‌دهی، مدیریت و اتصال داده‌ها در محصولات و خدمات Uniqe.",

      "Integrations":
        "یکپارچه‌سازی‌ها",

      "Infrastructure for connecting external services, applications and future components to Uniqe.":
        "زیرساخت اتصال سرویس‌ها، اپلیکیشن‌ها و اجزای آینده به Uniqe.",

      "Infrastructure":
        "زیرساخت",

      "The underlying technical foundation for reliable, scalable and connected Uniqe experiences.":
        "بنیان فنی برای ایجاد تجربه‌های قابل اعتماد، مقیاس‌پذیر و متصل در Uniqe.",

      "Future Interface":
        "رابط آینده",

      "Experimental interface concepts exploring new ways of interacting with digital products and experiences.":
        "مفاهیم آزمایشی رابط کاربری برای بررسی روش‌های جدید تعامل با محصولات و تجربه‌های دیجیتال.",

      "AI Experiments":
        "آزمایش‌های هوش مصنوعی",

      "Explorations around artificial intelligence, intelligent systems and new possibilities for digital products.":
        "بررسی هوش مصنوعی، سیستم‌های هوشمند و امکانات جدید برای محصولات دیجیتال.",

      "New Technology":
        "فناوری‌های جدید",

      "Experimental work investigating emerging technologies and how they can become useful parts of Uniqe.":
        "پژوهش‌های آزمایشی درباره فناوری‌های نوظهور و کاربردهای آن‌ها در اکوسیستم Uniqe.",

      "Open Concepts":
        "مفاهیم باز",

      "A space for early ideas, unconventional concepts and experiments that may evolve into future Uniqe projects.":
        "فضایی برای ایده‌های اولیه، مفاهیم متفاوت و آزمایش‌هایی که می‌توانند به پروژه‌های آینده Uniqe تبدیل شوند.",

      "Prototype":
        "نمونه اولیه",

      "Research":
        "پژوهش",

      "Concept":
        "مفهوم",

      "Active":
        "فعال",

      "My U":
        "My U",

      "Your space inside Uniqe.":
        "فضای شخصی شما در Uniqe.",

      "Your identity.":
        "هویت شما.",

      "Your personal space for profile, activity and future Uniqe services.":
        "فضای شخصی شما برای پروفایل، فعالیت‌ها و خدمات آینده Uniqe.",

      "Member since":
        "عضویت از",

      "Activity":
        "فعالیت",

      "Connected layers":
        "لایه‌های متصل",

      "Appearance":
        "ظاهر",

      "Control how the Uniqe interface looks.":
        "ظاهر رابط کاربری Uniqe را کنترل کنید.",

      "Language":
        "زبان",

      "Choose the language used across Uniqe.":
        "زبان مورد استفاده در Uniqe را انتخاب کنید.",

      "Notifications":
        "اعلان‌ها",

      "Manage future notifications and updates.":
        "اعلان‌ها و به‌روزرسانی‌های آینده را مدیریت کنید.",

      "Recently":
        "اخیراً",

      "Explore":
        "کشف",

      "Visit":
        "بازدید",

      "Project":
        "پروژه",

      "System":
        "سیستم",

      "Explored UApps":
        "UApps را بررسی کرد",

      "Visited UWeb":
        "از UWeb بازدید کرد",

      "Visited UShop":
        "از UShop بازدید کرد",

      "Visited USchool":
        "از USchool بازدید کرد",

      "Explored LAB":
        "LAB را بررسی کرد",

      "Opened My U":
        "My U را باز کرد",

      "All Activity":
        "همه فعالیت‌ها",

      "Visits":
        "بازدیدها",

      "Profile":
        "پروفایل",

      "Search":
        "جستجو",

      "Search results":
        "نتایج جستجو",

      "No results found":
        "نتیجه‌ای پیدا نشد",

      "Notifications Center":
        "مرکز اعلان‌ها",

      "All Notifications":
        "همه اعلان‌ها",

      "Unread":
        "خوانده‌نشده",

      "Mark as read":
        "علامت‌گذاری به‌عنوان خوانده‌شده",

      "Favorites":
        "علاقه‌مندی‌ها",

      "All Favorites":
        "همه علاقه‌مندی‌ها",

      "Remove from favorites":
        "حذف از علاقه‌مندی‌ها",

      "All":
        "همه",

      "Account":
        "حساب کاربری",

      "Log in":
        "ورود",

      "Create account":
        "ساخت حساب",

      "Log out":
        "خروج از حساب",

      "Something went wrong":
        "مشکلی پیش آمد",

      "We couldn't load this experience":
        "امکان بارگذاری این بخش وجود ندارد",

      "An unexpected error occurred. You can try again or return to the Uniqe home page.":
        "یک خطای غیرمنتظره رخ داد. می‌توانید دوباره تلاش کنید یا به صفحه اصلی Uniqe بازگردید.",

      "Try again":
        "تلاش دوباره",

      "Go home":
        "بازگشت به خانه",

      "Critical Error":
        "خطای بحرانی",

      "Uniqe needs a restart":
        "Uniqe نیاز به راه‌اندازی مجدد دارد",

      "A critical application error occurred. Please try again.":
        "یک خطای مهم در برنامه رخ داده است. لطفاً دوباره تلاش کنید.",

      "Restart":
        "راه‌اندازی مجدد",

      "Not Found":
        "یافت نشد",

      "This page doesn't exist":
        "این صفحه وجود ندارد",

      "The page you are looking for may have moved, changed, or never existed.":
        "صفحه‌ای که به دنبال آن هستید ممکن است جابه‌جا شده، تغییر کرده یا اصلاً وجود نداشته باشد.",

      "Loading":
        "در حال بارگذاری",

      "Preparing Uniqe":
        "در حال آماده‌سازی Uniqe",

      "Please wait while the next experience is being prepared.":
        "لطفاً منتظر بمانید تا بخش بعدی آماده شود.",

      "Offline":
        "آفلاین",

      "You're currently offline":
        "در حال حاضر آفلاین هستید",

      "Check your internet connection and try again when you're back online.":
        "اتصال اینترنت خود را بررسی کنید و پس از اتصال دوباره تلاش کنید.",

      "Maintenance":
        "تعمیر و نگهداری",

      "Uniqe is being updated":
        "Uniqe در حال به‌روزرسانی است",

      "We're working on the platform. Please check back soon.":
        "در حال کار روی پلتفرم هستیم. لطفاً بعداً دوباره مراجعه کنید.",

      "Central API":
        "API مرکزی",

      "Connected":
        "متصل",

      "Backend not connected":
        "Backend متصل نیست",

      "One space. Your Uniqe.":
        "یک فضا. Uniqe شما." 
            "Users":
        "کاربران",

      "Registered users":
        "کاربران ثبت‌نام‌شده",

      "Products":
        "محصولات",

      "Courses":
        "دوره‌ها",

      "Projects":
        "پروژه‌ها",

      "Applications and app projects":
        "اپلیکیشن‌ها و پروژه‌های نرم‌افزاری",

      "Core services and infrastructure":
        "سرویس‌های مرکزی و زیرساخت",

      "Experiments and research":
        "آزمایش‌ها و پژوهش‌ها",

      "Ready":
        "آماده",

      "Add Product":
        "افزودن محصول",

      "Create a new UShop product":
        "ایجاد محصول جدید در UShop",

      "Manage Courses":
        "مدیریت دوره‌ها",

      "Open USchool management":
        "باز کردن مدیریت USchool",

      "Manage Users":
        "مدیریت کاربران",

      "View and manage users":
        "مشاهده و مدیریت کاربران",

      "Settings":
        "تنظیمات",

      "System configuration":
        "تنظیمات سیستم",

      "Dashboard":
        "داشبورد",

      "Welcome to the central administration panel of Uniqe.":
        "به پنل مدیریت مرکزی Uniqe خوش آمدید.",

      "System Online":
        "سیستم آنلاین",

      "ECOSYSTEM":
        "اکوسیستم",

      "Uniqe Modules":
        "ماژول‌های Uniqe",

      "QUICK ACCESS":
        "دسترسی سریع",

      "Quick Actions":
        "عملیات سریع",

      "SYSTEM ACTIVITY":
        "فعالیت سیستم",

      "Recent Activity":
        "فعالیت‌های اخیر",

      "LIVE":
        "زنده",

      "No activity yet":
        "هنوز فعالیتی ثبت نشده است",

      "System activity will appear here once users and modules start generating events.":
        "پس از شروع فعالیت کاربران و ماژول‌ها، فعالیت‌های سیستم در اینجا نمایش داده می‌شوند.",
    },
  },
} as const;

export type TranslationDictionary =
  (typeof translations)[Language];

export type TranslationTextMap =
  Record<string, string>;